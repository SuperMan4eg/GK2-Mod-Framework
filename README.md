...
- Mouse input is verified. Gamepad-mode navigation is runtime-tested from the mod list through per-mod Settings controls, including toggles, sliders, exact numeric-field focus, dropdowns, keybinds, text activation, action buttons, per-row Reset paths, Reset/rebuild focus, conditional hidden/disabled rows, Settings-page return focus, and the scaled `1600x900` path. Automated probes use the game's real navigation controller/input mode; physical controller spot-checks remain useful device/mapping coverage.
